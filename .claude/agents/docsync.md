...
git log --oneline --since="$(cat .claude/workspace/.last-sync-date 2>/dev/null)" \
  --name-only -- '*.go' '*.ts' '*.py' '*.rb' | head -100
```

### Bước 2: Kiểm tra từng Tài liệu Phân tích

Với mỗi file trong `.claude/workspace/analysis/`:
...
Tạo file `.claude/workspace/drift-{date}.md`:
...
```bash
date -u +"%Y-%m-%dT%H:%M:%SZ" > .claude/workspace/.last-sync-date
```

### Bước 6: Commit
```bash
git add .claude/workspace/analysis/ .claude/workspace/drift-*.md \
         .claude/workspace/.last-sync-date PROJECT-PROFILE.md
...
Cảnh báo được ghi vào `.claude/workspace/STALE-ALERTS.md` và in ra terminal.
